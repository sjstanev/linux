...
if [ $app == 'curl' ]; then
  curl --request POST --url http://10.81.0.10/jabber// --header 'content-type: application/json' --data @/tmp/hw.log
else
  wget --quiet --method POST --header 'content-type: application/json' --body-file=/tmp/hw.log --output-document - http://10.81.0.10/jabber//
fi